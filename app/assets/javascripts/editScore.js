...
//    //if (e.button == 2) {
//    for (var i = notes.length - 1; i >= 0; i--) {
//      var aaa = (notes[i] & 0x3F)
//      var bbb = aaa |= 0x80;
//      console.log( aaa |= 0x80 , GridY)
//      if ((notes[i] & 0x3F) == GridY || bbb == GridY) {
//        notes.splice(i, 1);
//        Score.notes[Track.track-1].note[scrollGridX] = notes;
//        break;
//      }
  ...
  for (var i=0; i<GridCount+1; i++) {
    ...
    for(var j = 0; j < Score.notes.length; j++){
      ...
      for (var k = 0; k < b.length; k++) {
        ...
        Layer.fillStyle = Color[Track.index[j%7]-1];
...
window.addEventListener('load', function(){
  ...
  $('#Measure').on('change', function(){
    ...
    UI.scrool.max = ScrollMax = Score.beat - GridCount;
    UI.scrool.disabled = false;
    animeStop();
  });
  ...
  //Track
  Track.index = $("[name=show]").map(function(){ return $(this).val() }).get();
